...
// Email a quote to a client as a share LINK (not a PDF attachment) — the share
// page is interactive and always reflects the operator's latest edits. Mirrors
// InvoiceEmailModal: pre-fills To with the linked contact's email, sends via
// Resend with the operator's account email as Reply-To, and auto-flips the
// quote draft → sent on success.
export default function QuoteEmailModal({ quote, onClose, onSent }) {
  const contactEmail = quote.contact?.email || '';
  const [to, setTo] = useState(contactEmail);
  const [cc, setCc] = useState('');
  const [subject, setSubject] = useState(`Your proposal${quote.title ? ` — ${quote.title}` : ''}`);
  ...
  const handleSend = async () => {
    ...
    setSending(true);
    try {
      await api.post(`/quotes/${quote._id}/email`, { to, cc, subject, message });
      toast.success('Quote emailed');
      onSent?.();
    } catch (err) {
      if (err.response?.data?.code === 'BLOCKING_CONDITIONS_UNACKNOWLEDGED') {
        toast.error('Acknowledge the blocking conditions before sending.');
      } else {
    ...
    <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-card rounded-xl border border-border w-full max-w-lg" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between p-4 border-b border-border">
          <h2 className="text-base font-semibold text-foreground">Email quote{quote.quoteNumber ? ` #${quote.quoteNumber}` : ''}</h2>
        ...
        <div className="p-4 space-y-3">
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="A personal note — appears above the link to the proposal."
              className="w-full px-3 py-2 rounded-lg border border-border bg-background text-sm"
            />
          </div>
          <p className="text-xs text-muted-foreground">
            A secure link to the live proposal is included automatically — no attachment, so the client always sees your latest version.
          </p>
        </div>
